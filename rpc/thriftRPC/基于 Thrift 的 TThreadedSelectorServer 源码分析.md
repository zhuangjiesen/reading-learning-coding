...
### Thrift 问题
1. Thrift 对于JDK Nio 的 Selector 的空轮询导致cpu 100% 的问题没有进行处理；
2. Thrift 需要对 AbstractNonblockingServer.java 类  maxReadBufferBytes(缓冲区内存大小限制) 设置 ,否则将导致 heap 区 oom问题；
3. 由问题 2 ，看了thrift 源码，在acceptor线程的时候，我认为需要有个接口来进行黑名单拦截，或者协议过滤，而不是到达 FrameBuffer 的read 时候才进行处理，
